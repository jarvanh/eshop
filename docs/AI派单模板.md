...
- 单测硬性要求：支付幂等 / 优惠管道 / 协同域 / 提现域四个核心域必须带测试
...
- 推送时机：每次 commit 后立即 push 到 origin；会话/工作日收尾前必须 push，
  工作区保持干净（git status 无未跟踪的代码文件）
...
- 完成动作：单测通过 → commit（feat(store): A-05 ...）→ push →
  跟踪表 A-05 状态改「已完成」+ 备注
...
- **会话收尾固定动作**：让 AI 输出三件套——①完成的 ID 清单 ②commit+push 记录（git log 摘要）③下一步建议；你与跟踪表对账，不符即纠正
- **周例会**：过一遍跟踪表「进行中」项 + git log 周报
